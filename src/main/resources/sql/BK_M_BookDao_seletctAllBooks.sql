/* BK_M_BookDao_seletctAllBooks.sql */
select
    B.BOOK_ID,
    B.TITLE,
    B.TITLE_KANA,
...
from
    ...
    left join (
        ...
        from
            book_db.BK_T_LEND join book_db.M_MEM_BASIC
                on book_db.BK_T_LEND.MEM_ID = book_db.M_MEM_BASIC.MEM_ID
        where
            LEND_STATUS = 11
    ) as LM
        on B.BOOK_ID = LM.BOOK_ID
    left join (
        select
            BOOK_ID,
